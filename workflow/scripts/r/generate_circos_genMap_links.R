...
# Outputs
gm_kar = snakemake@output[[1]]
match_gm_links = snakemake@output[[2]]
nomatch_gm_links = snakemake@output[[3]]
markerPos= snakemake@output[[4]]
...
filtered_markers <- genMap_df_mod %>% 
  ...
  # Filter to only include a single mapping pop
  filter(pop == 'DG') %>%
...
LG_karyotype <- filtered_markers %>% 
  ...
  distinct()
write_delim(LG_karyotype, gm_kar, delim = '\t', col_names = F)

# Keep only markers with alignments to both references
markers_toKeep <- dplyr::intersect(
  filtered_markers %>% filter(ref == 'TrRv5') %>% dplyr::select(name),
  filtered_markers %>% filter(ref == 'utm') %>% dplyr::select(name)
) %>%
  pull(name)
...
utm_genMap_links <- filtered_markers %>% 
  filter(ref == 'utm' & name %in% markers_toKeep) %>% 
  mutate(LG = paste0("LG_", LG),
...
TrRv5_genMap_links <- filtered_markers %>% 
  filter(ref == 'TrRv5' & name %in% markers_toKeep) %>% 
  mutate(LG = paste0("LG_", LG),
         cspos = chrom_pos, cepos = chrom_pos,
         lspos = lg_pos, lepos = lg_pos) %>% 
...
allLinks <- bind_rows(utm_genMap_links, TrRv5_genMap_links)
matchLinks <- allLinks %>% 
  filter(LG == corr_LG) %>% 
  dplyr::select(sseqid, cspos, cepos, LG, lspos, lepos)
write_delim(matchLinks, match_gm_links, col_names = FALSE, delim = '\t')

noMatchLinks <- allLinks %>% 
  filter(LG != corr_LG) %>% 
  dplyr::select(sseqid, cspos, cepos, LG, lspos, lepos)
write_delim(noMatchLinks, nomatch_gm_links, col_names = FALSE, delim = '\t')
  
# Dataframe with marker positions
marker_pos <- utm_genMap_links %>% 
  dplyr::select(LG, lspos, lepos)
write_delim(marker_pos, markerPos, col_names = FALSE, delim = '\t')
